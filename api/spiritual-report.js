// /api/spiritual-report.js — FINAL: short answer + email + KV premiumToken
...
export default async function handler(req, res) {
  ...
  let palm1File = null;
  let palm2File = null;
  let uploadedFile = null; // legacy technical file support
  ...
  if (mode === "personal") {
    ...
    if (files?.technicalFile) {
      uploadedFile = Array.isArray(files.technicalFile)
        ...
        : files.technicalFile;
      // you can validate uploadedFile if you want
    }
  }
  ...
  try {
    ...
    console.error("runAllEngines error:", err);
    return res
      .status(500)
  ...
  /* PREMIUM TOKEN — save to KV for later upgrade */
