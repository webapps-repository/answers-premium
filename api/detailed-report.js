// /api/detailed-report.js — PREMIUM EMAIL ONLY (KV SAFE MODE)
...
export default async function handler(req, res) {
  ...
  if (!cached) {
    return res.status(404).json({ error: "Premium token expired or invalid" });
  }
  ...
  const question =
    (fields.question &&
      (Array.isArray(fields.question) ? fields.question[0] : fields.question)) || "";

  if (!email) {
    return res.status(400).json({ error: "Email missing in original submission" });
  }

  /* ✅ PREMIUM EMAIL (NO PDF YET – STABLE MODE) */
  const html = `
